...
alter table app_public.owners
    add marketing_source uuid default null;

alter table app_public.owners
    add constraint owners__fk_marketing_source
        foreign key (marketing_source) references app_public.marketing_sources (uuid) on delete set null;
...
create policy select_own on marketing_sources for select using (
    exists(
            ...
            where owners.uuid = current_owner_uuid()
              and owners.marketing_source = marketing_sources.uuid
        )
    );
grant select on marketing_sources to asyncy_visitor;

drop function app_private.create_owner_by_login(app_public.git_service, text, username, text, email, text, text);
